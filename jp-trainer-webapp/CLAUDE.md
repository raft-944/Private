...
句型库不按教材分类，直接按 JLPT 等级(N5/N4/N3/N2/N1)组织，现覆盖 N5-N2 共 480+ 句型，
正在往 N1 逐级扩充。N2 内容(`src/data/n2-part1.js`)是从用户上传的 N2 语法讲义 docx
提取转换的，讲义原文的"辨析·注意点"是自然语言段落、没有拆成结构化的对比数据，
所以整段并入了 explain 字段，这些句型的 contrasts 数组暂时是空的；讲义里"文法形式の
整理"归类总结表和"文の組み立て/文章の文法"两个写作技巧部分，内容不是单个可操练的
句型卡片格式，没有转换收录。除了核心的句型 SRS 循环，还有練習帳(知识辨析/场景对话/书面邮件，
