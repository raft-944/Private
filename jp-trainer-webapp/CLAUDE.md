...
- **判卷结果必须自洽,不能自相矛盾**。目前有两道互补的检查,都不要拿掉:①`selfCheck`(AI 自己回头看一遍)——讲评里挑了毛病却判了 `correct`,就置 false;②`reconcileGradeReference()`——判了 `partial`/`wrong`,但 `reference` 和学生答案其实是同一句话(`sameJaText` 归一化后比较)。第二种是 2026-08 真实反馈:学生写「夏休みは海へ旅行に行こう」,判 partial、讲评说该改成「海へ旅行しよう」,但 reference 原样抄了学生的句子——学生看到的就是"参考答案和我写的一模一样,凭什么判我只是接近"。修法是提示词(`REFERENCE_MUST_DIFFER_RULE`,凡是带 reference 的 grader 都要插)+ 客户端兜底两手:兜底会把 verdict 提成 `correct`(答对的句子不该缩短复习间隔),同时把 `selfCheck` 置 false 复用"建议复核"通道,让它照样留在错题本里等人工确认。**注意 `reconcileGradeReference` 要在 `normalizeErrorScope` 之前调用**,否则 verdict 改了而 errorScope 还停在旧值上。
