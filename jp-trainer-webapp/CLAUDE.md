...
No test runner or lint script is configured in `package.json`. 只有四个针对性的验证脚本:`npm run test:grading`(判卷回归,要 `DEEPSEEK_API_KEY`,真的花钱)、`npm run test:mistakes`(錯題本的容量/淘汰/挑题顺序)、`npm run test:qbank`(题库的复现/入库/回填/降级,靠拦截 `/api/generate` **数出题调用次数**——"有没有省下调用"正是这个功能的全部意义)和 `npm run test:secondopinion`(二次核验两个方向的合并规则,拦 `/api/generate` 分别控制第一次判卷和第二次复核各返回什么)。后三个不调 AI、不花钱,秒级跑完。四个都是起 vite + Playwright、在页面里 import 真正的 `App.jsx` 调它 `export` 出来的函数,**不要在脚本里另抄一份实现**。四个脚本都需要一份(内容随便的)`.env`,因为 `App.jsx` 无条件 import `supabaseClient`;沙箱里没有匹配版本的 Playwright 浏览器时用 `CHROMIUM_PATH=/opt/pw-browsers/chromium` 指过去。
...
- **判卷质量的三道防线**(自洽检查只能发现"判定和讲评互相打架",发现不了"AI 的日语判断本身就错了",所以另外加了三条):①**自动二次核验**(`secondOpinion`,触发条件见 `secondOpinionDirection`)——**两个方向都核**,都换 pro 模型、**不告诉复核方第一次判了什么**(告诉了就会锚定成附和,和 `verifyGrammarChoiceAnswers` 同一套路)。**判太严**(`verdict === "wrong"`,代价最大:间隔砍回最短+计入 `missTotal`)两次不一致取**较宽松**的一侧;**判太松**(`verdict === "correct"` 且 `selfCheck === false`,2026-08 补上的另一半——在那之前防线只挡判太严,而实际遇到的误判恰恰都是判太松:多余的「の」被讲评说成"名词化可以接受"、假名写法被当成量词用错)复核认为不对时**降级到 `partial` 封顶**,不直接按复核的 `wrong` 算——第一位判 correct、第二位判 wrong 差着两档,证据强度撑不起最重的惩罚;复核也认可时判定不动,只在讲评里追加一句让你能放心点"确认无误"。不对所有 `correct` 复核:那等于每题翻倍花钱,而绝大多数 correct 是干净的。两个方向都置 `selfCheck=false` 留痕,核验本身失败一律沿用原判。**复核结论走独立字段 `g.review = {first, second, reason}`,绝不拼进 `explanation`**——最早那版是往讲评正文尾巴上接一段「【复核】…」,结果两段话在同一个框里抢注意力,而复核给的理由和初核讲的往往是同一个错(实际截图:初核说「開く」该改成「開いている」,复核又原样说一遍),讲评「言简意赅、直指错误」的初衷就没了。现在由 `<ReviewNote>` 单独渲染成一条低一档的小字条(讲评列表/组页面/錯題本三处),`db.mistakes` 和组内 `rec` 也各存一份 `review`。合并规则涉及排期,改动要跑 `npm run test:secondopinion`(拦 `/api/generate` 分别控制两次调用各返回什么,不花钱)。②**用户一键报错**(`ReportGradeBtn` → `db.gradeReports`,上限 `GRADE_REPORT_MAX`)——判卷结果下面的低调按钮,点一下把完整上下文(含 `verdict`/`errorScope`/`selfCheck` 这些界面上看不到、但排查时最关键的字段)存进 db,在「我的」里可查看和一键复制成纯文本。只记录,不影响判定和排期。③**回归测试**(`scripts/grade-regression.mjs`)——把历次真实 bug 固化成用例,`DEEPSEEK_API_KEY=xxx node scripts/grade-regression.mjs` 跑一轮约一毛钱。它起 vite + Playwright、在页面里 import 真正的 `App.jsx` 调 `gradeAnswer`(所以 `gradeAnswer` 是 `export` 的,运行时没人 import 它),测的就是线上跑的那份代码含所有兜底,**不要在脚本里另抄一份提示词**——抄了就会各走各的,测过也不作数。用例的 `expect` 断言"不该发生什么"而不是"必须等于某个值",判卷本有主观空间,卡死唯一答案会让测试天天飘红最后没人看。**改判卷提示词之后应该跑一遍**。
