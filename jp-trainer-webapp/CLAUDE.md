...
- **我的** (`view: "account"`) — account hero (email + a free-quota progress bar), the user manual, 账户设置 (change password/email, plain `supabase.auth.updateUser` — the same call `ResetPasswordScreen` in `main.jsx` uses), and 退出登录 (moved here from 首页's 设置与备份, which now holds only settings + progress export/import). Email and quota load lazily on entering the view as **two independent states** (`acctEmail` / `acctQuota`), deliberately not one combined object: the password/email forms only need `supabase.auth` and must stay usable when the `get_my_usage` RPC fails (e.g. the quota SQL hasn't been applied to that deployment yet) — an earlier single-state version blanked the whole page on any one failure.
