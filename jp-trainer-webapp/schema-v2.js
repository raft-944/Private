...
/* ---------- 字段定义 ----------
 * book      String  教材标识,见 patternsData.js 的 BOOKS（如 "minna"｜"biaori"）。
 *                   由 patternsData.js 在拼装 PATTERNS 数组时统一打上，不写在各数据文件里。
 * lesson    Number  课号，教材内部自己的编号（不同教材各自从头编，book 字段已经把
 *                   两本书的句型分开，课号不需要跨教材保持唯一）。大家的日语：初级 1-50，
 *                   中级 51-62（= 50 + 中级课号）。
 * level     String  "初級" | "中級"，用于出题难度基准，避免 AI 停留在 N5-N4
 * style     String  可选。文体归属 "敬体" | "简体" | "两可"，省略等同「两可」。
 *                   只标注真正受限的句型（例如「〜ていただけませんか」形式本身自带
 *                   ます／でしょう，只能是敬体），出题时约束例句、判卷时约束答案。
 *                   过度标注会产生不必要的限制，让 AI 出的题变别扭，故宁缺勿滥。
 * formality String  可选。语域 "书面语" | "口语" | "两可"，省略等同「两可」。
 *                   依据教材 explain 里明确写了语体限制的句型来标（例如「〜みたいだ」
 *                   教材注明「属于口语形式，不用于正式文书、演讲稿或作文」）。
 *                   注意：与 data/scenes.js 里场景的 register 字段不是一回事——
 *                   那个指对话双方关系亲疏，这里指句型本身的语体，故不复用同一名字。
 * pattern   String  句型本身
 * conn      String  接续方式
 ...
 * study     Object  可选。教材式的详细讲解，只给教材外的补充句型写（教材内的句型
 *                   课本上就有，不需要）。内容不写在这个文件里，而是集中在
 *                   data/extStudyNotes.js，由 patternsData.js 按 pattern 字符串合并进来。
 *                   结构：{ form: 接续说明, usages: [{use, ex: [[日,中],…]}], notes: [注意] }
 *                   注意别和 explain 混为一谈：explain 是「给 AI 看的」，判卷截 300 字、
 *                   出题截 160 字，写长了会被从中间截断；study 是「给人看的」，
 *                   只在界面上展示，不进任何提示词，所以可以按课本的篇幅写全。
 */
