...
const CASES = [
  ...
  {
    name: "没有文体问题时讲评里不该提文体",
    note: `2026-08 真实反馈:「来月から残業をしないことになります」整句都是敬体、跟文体毫无关系,
           讲评却写"虽未用敬体,但该句型本身不带文体限制"——既多余又是错的(「なります」就是敬体),
           而且把 120 字讲评占掉之后,真正该讲的「なります vs なりました」时态差别一个字没提。
           先加了通用的 EXPLANATION_FOCUS_RULE 没压住,又在 STYLE_CONSISTENCY_RULE 末尾钉了
           "没查出问题就一个字都别提文体"。`,
    pattern: "Vることになりました",
    task: "从下个月开始不用加班了。",
    answer: "来月から残業をしないことになります",
    expect: (g) => /敬体|简体|敬體|簡體|文体|文體/.test(g.explanation || "")
      ? "这句话文体统一,讲评却提到了文体"
      : null,
  },
  {
