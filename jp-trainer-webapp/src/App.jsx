...
/* 判卷时统一插入的"文体前后一致"规则。不依赖句型有没有标签——查的是学习者自己
   ...
   末尾那条"没查出问题就一个字都别提文体"是 2026-08 又补的,别删:通用的
   EXPLANATION_FOCUS_RULE 上线之后,文体这条仍然压不住——真实 case 是
   「来月から残業をしないことになります」(整句都是敬体、跟文体毫无关系),讲评却写
   "虽未用敬体,但该句型本身不带文体限制"。这句话不只是多余,它还是错的(「なります」
   本来就是敬体),而且 120 字的讲评被它占掉之后,真正该讲的
   「なります vs なりました」的时态差别一个字没提。
   通用纪律管的是"别复述无关规则",而模型对这条最容易破例,所以要在这条规则自己
   里面再钉一遍——针对性的禁令比通用提醒有力得多。 */
...
**没查出文体问题时,explanation 里一个关于文体的字都不许出现。**不许写"文体统一""虽未用敬体""该句型本身不带文体限制""文体没有问题"这类话——哪怕是肯定的说法也不行。这条检查是给你自己判卷用的,不是讲评要交代的内容;学生只想知道这道题他哪里写得好、哪里该改,不想知道你检查过哪些清单。(顺带记牢:「〜ます」「〜です」结尾的句子**就是敬体**,不要把它说成"未用敬体";「なります/なりました」「します/しました」这种差别是**时态**,两个都是敬体,不许拿"敬体"去给一个时态错误命名。)`;
...
- "outside": 目标句型本身用得完全正确,错误只出在句型之外的地方(单词选错/写错、与句型无关的助词、量词(助数詞)搭配不当、时态、拼写、汉字错字等),且这个词换错之后不影响这句话原本要表达的方向/语义关系
...
/* 判卷时统一插入的"参考答案要和判定对得上"规则。两个方向的自相矛盾都真实发生过,
   两条都不能删:

   ①判了 partial,reference 却原样抄学生的句子。学生写「夏休みは海へ旅行に行こう」,
     讲评说「旅行に行こう」啰嗦、该改成「海へ旅行しよう」,但 reference 一字未改。
     学生看到的是"标准答案就是我写的这句,凭什么说我只是接近"——根本讲不通。
   ②判了 correct,reference 却偷偷改掉了学生的一个词,讲评还说"完全正确、无需修改"。
     学生写「毎日卵を6枚食べることにしました」(鸡蛋的量词该用「個」不是「枚」),
     reference 写的是「6個」,讲评却夸他"「6枚」的用法也自然"。学生追问一句,AI 立刻
     承认「6枚」不对——也就是说它其实知道,只是判卷时没把这个认知落到 verdict 上。 */
const REFERENCE_CONSISTENCY_RULE = `参考答案(reference)和判定必须对得上,这是硬性要求:
...
- **反向同样禁止**:如果你在 reference 里改动了学生句子里的任何一个词(哪怕只是换了个助词、换了个量词、换了个汉字写法),就说明你认为学生那里写得不对或不自然。这种情况下**不可以**判 "correct" 然后在讲解里说"完全正确、无需修改"——要么如实判 "partial" 并在讲解里说清楚改了什么、为什么改;要么如果学生的写法其实也完全成立(只是你个人偏好另一种说法),那就把 reference 改回和学生一致、或在讲解里明确说明"你的写法也正确,下面只是另一种说法"。绝不允许出现"参考答案偷偷改了一个词、讲评却说无需修改"的情况。
- 特别注意**量词(助数詞)**:量词用错是很容易被轻轻放过、但确实算错的一类问题(比如鸡蛋、水果这类小而立体的东西用「個」,「枚」只用于纸张、盘子、邮票这类薄而平的东西;还有 本/冊/匹/頭/杯/台 等各有各的适用对象)。发现量词搭配不当,要按错误处理并在讲解里指出,不能因为"整句读起来通顺"就判全对。`;

/* 判卷尺度的一致性/公平性规则。起因又是一个真实反馈,而且这次是"同样的结构判了两种结果":
   题A「请给每名学生各发三支铅笔」,学生写「みんなに三本ずつ鉛筆を配ってください」→ 判正解,
   AI 自己的参考答案也是「学生に三本ずつ鉛筆を配ってください」(同一个语序);
   题B「请用ずつ造句」,学生写「こどもに2枚ずつ寿司を配ってください」→ 判 wrong,
   理由之一竟然是"「2枚ずつ寿司」的语序不对,应改成「寿司を2枚ずつ」"——可这正是它上一题
   自己写出来并判对的语序。学生的原话:"为什么我用同样的句型顺序,一个是对的一个是错的?"

   两个毛病要一起治:
   ①凭空发明规则:日语谓语之前的成分语序本来就相当自由,把"我更习惯的语序"说成语法错误;
   ②判罚过重:题B真正的问题只有量词(寿司不用「枚」),句型 ずつ 本身用得完全正确,
     按下面 verdict 的定义这最多是 partial,却给了 wrong。 */
const GRADING_FAIRNESS_RULE = `判卷尺度(很重要,这几条直接关系到判卷可不可信):
- **日语谓语之前的语序是自由的**。同一句话里的主题、对象、数量词、目的语,只要助词标对了,前后调换位置都成立,意思也不变——例如「子供に2枚ずつ寿司を配る」和「子供に寿司を2枚ずつ配る」是同样正确的两句话,数量词放在名词前面还是后面都可以。绝对不能因为学生写的语序和你习惯的不一样就判错、降级、或者在讲解里说他"语序有问题"。只有当语序真的导致语法不成立或意思变了,才算错。
- **不要凭空发明规则**。你在讲解里指出的每一条问题,都必须是真正的语法/用法错误,而不是"我更喜欢另一种说法"。如果你说不出这样写为什么在语法上不成立,那它就不是错误——顶多在讲解里以"也可以这样说"的形式提一句,不能据此扣分。
...
/* 讲评的输出纪律,压在所有判卷规则的最后。
   ...
   "哪怕写成表扬也算罗列"是 2026-08 回归测试跑出来后补的,别删。原来三条子句全是
   否定式的("不要罗列学生没有犯的错"),而模型判 correct 时是把检查清单**当成表扬
   逐条念出来**的——「量词搭配得当」「接续也正确」「文体统一」,字面上没在罗列错误,
   通用纪律就拦不住。实测一轮 10 条用例里 3 条讲评提到文体,其中一条原话就是
   STYLE_CONSISTENCY_RULE 结尾逐字点名禁止的「文体统一」;而那道题(どのかい/回)四句
   讲评里三句是清单复述,120 字就是这么被吃光的。堵法只能是把"肯定的说法"也一并禁掉。 */
...
- 不要罗列学生**没有**犯的错,也不要写与本题无关的泛泛提醒;**把检查项写成表扬,同样算罗列**——「量词搭配得当」「接续也正确」「语序自然」「文体统一」这类肯定的点评一句都不许写。
判对了**只说这道题的目标句型为什么用对了**,一两句话打住,不要再逐条交代别的检查项过没过;判得不对就直接指出错在哪、该怎么改。宁可短,不要为了凑字数把无关的规则复述一遍。`;

/* 判卷结果的自洽性兜底。提示词里已经明令禁止上面那两种矛盾(见 REFERENCE_CONSISTENCY_RULE),
   这里是它没听话时的兜底,两个方向各处理一种:

   ①判了 partial/wrong,但 reference 和学生答案其实是同一句话。逻辑上讲不通——参考答案
     就是标准答案,标准答案等于学生的答案,那学生就是对的。处理:升级成 correct(不能让
     一个答对的句子去缩短复习间隔,那是实打实的损失),同时把 selfCheck 置 false。
   ②判了 correct,但 reference 把学生的句子悄悄改了一两个字符。这种"极小的改动"几乎
     一定是在纠错(枚→個、は→が、に→で 这类),而不是另起一句同义的说法——真正的
     "另一种自然说法"通常会差很多字。这种情况不动 verdict(前端没有能力判断到底谁对,
     瞎改反而更糟),只把 selfCheck 置 false 引起注意。
     阈值卡在"改动不超过3个字符"上:改得多的那些大概率就是正常的参考答案不同写法,
     全都标出来会把错题本刷爆,反而没人看了。

   两种情况都复用已有的"建议复核"通道(selfCheck=false):这道题照样留在错题本里标着
   "⚠️ 建议复核",讲评原样显示,自己扫一眼就能确认到底谁对。
   只有在 reference 确实存在时才做判断——听力题(gradeListening)没有 reference 字段。 */
const SILENT_FIX_MAX_EDITS = 3;
function reconcileGradeReference(g, answer) {
  if (!g || !g.reference || !answer) return g;
  const same = sameJaText(g.reference, answer);
  if (g.verdict !== "correct") {
    return same ? { ...g, verdict: "correct", errorScope: "none", selfCheck: false } : g;
  }
  if (same) return g;
  const edits = editDistance(normalizeJaText(g.reference), normalizeJaText(answer));
  return edits > 0 && edits <= SILENT_FIX_MAX_EDITS ? { ...g, selfCheck: false } : g;
}

/* 标准的 Levenshtein 编辑距离,只用来量"参考答案和学生答案差了几个字符"。
   滚动数组实现,句子都很短(几十个字符),开销可以忽略。 */
...
function sameJaText(a, b) {
  return normalizeJaText(a) === normalizeJaText(b);
}
...
- verdict 不是 "correct",reference 却和学生的答案一模一样。
这个审视过程只在你内部完成,不要把思考过程写出来,直接根据结果给出最终JSON。
...
输出JSON(直接输出,不要有任何前缀说明或思考文字): {"verdict":"correct|partial|wrong","selfCheck":true|false,"reference":"一个自然的参考答案(日语,需同时包含两个句型);verdict 不是 correct 时必须是改过之后的句子、不能和学生答案一模一样;反过来,只要你在这里改动了学生的任何一个词,就不能判 correct+无需修改","explanation":"分别点评两个句型各自的使用情况,指出哪里好、哪里需要改,用中文,150字以内","breakdown":{"skeleton":"...","verbForm":"...","particleReason":"...","modifier":"..."}或null}`;
...
注意:explanation 字段必须全部用中文写,绝对不可以用日语写讲解,引用日语词汇/例句除外。
输出JSON(直接输出,不要有任何前缀说明或思考文字): {"verdict":"correct|partial|wrong","selfCheck":true|false,"errorScope":"none|pattern|outside|both","reference":"一个自然的参考答案(日语);verdict 不是 correct 时必须是改过之后的句子、不能和学生答案一模一样;反过来,只要你在这里改动了学生的任何一个词,就不能判 correct+无需修改","explanation":"针对学生答案的具体讲解,指出好在哪/错在哪及如何改,用中文,120字以内","breakdown":{"skeleton":"...","verbForm":"...","particleReason":"...","modifier":"..."}或null}`;
  let g = await callAI(sys, user);
  if (!g.verdict) throw new Error("bad grade");
  if (typeof g.selfCheck !== "boolean") g.selfCheck = true;
